...
class Inventario:
    ...
    def añadir_producto(self, producto=None):  # Se puede ignorar el parámetro
    ...
    def guardar_en_excel_xlsx(self, nombre_archivo):
        if os.path.exists(nombre_archivo):
            ...
            ws = wb.active
            ws.title = "Inventario"
            ws.append(["ID", "Nombre", "Cantidad", "Precio"])  # Encabezado solo si es nuevo

        # Añadir productos al final
    ...
    def eliminar_producto_en_excel(self,nombre_archivo, hoja, id_a_eliminar):
        ...
        for fila in range(2, ws.max_row + 1):  # Asumiendo que la fila 1 tiene encabezados
    ...
    def mostrar_todos_los_productos(self, nombre_archivo,hoja):
        ...
        for fila in range(2, ws.max_row + 1):  # Desde la fila 2 (asumiendo encabezados en la fila 1)
